  ...
  var ProductManager = (function(){
    ...
    */
    ...
    var updatePoduct = function(id, quantity) {
      ...
      products[productIndex].quantity = typeof quantity === "undefined" ? products[productIndex].quantity * 1 + parseInt($('#quantity_'+id).val()) : quantity;
      setAllProducts(products);
      return true;
    }
  ...
  var loadMyCartEvent = function(userOptions){
    ...
    if(!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '</div>' +
        '<div class="modal-body table-responsive scroll_class">' +
        '<table class="table table-hover" id="' + idCartTable + '"></table>' +
        '</div>' +
        '<div class="modal-footer">' +
        '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>' +
        '<button type="button" id="checkout_button" class="btn btn-primary ' + classCheckoutCart + '">Checkout</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
  ...
  var MyCart = function (target, userOptions) {
    ...
    */
    $target.click(function(){
      ...
      var quantity = parseInt($('#quantity_'+id).val()); //$target.data('quantity');
